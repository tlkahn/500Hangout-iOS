...
class SplitPaneViewController: BaseContainerViewController {
  ...
  fileprivate var rightController: UIViewController?
  fileprivate let wrapperView = UIView()
  ...
  func pushViewController(_ rightController: UIViewController, animated: Bool = true) {
  ...
  fileprivate static func stateForTraitCollection(_ traitCollection: UITraitCollection,
  ...
  fileprivate func transitionToState(
    _ state: State, forceCollapsed: Bool = false, overrideSize: CGSize? = nil, animated: Bool = true,
    ...
    completion: @escaping () -> Void) {
      ...
      if animated {
        ...
        if let coordinator = coordinator {
          ...
          UIView.animate(withDuration: 0.367, animations: animation, completion: { (finished) in
            finish()
          }) 
        }
      } else {
  ...
  /// - parameter overrideSize Override the size to use for layout.
  fileprivate func layoutViewControllersForState(_ state: State, forceCollapsed: Bool = false,
  ...
  fileprivate static func layoutViewControllersForState(_ state: State,
                                                    collapsed: Bool,
                                                    size: CGSize,
...
//                   .minXEdge)
      let (leftFrame, rightFrame) = wrapperView.bounds.divided(atDistance: wrapperView.bounds.size.width/2, from: .minXEdge)
...
//      CGRectDivide(fullBounds, &leftFrame, &rightFrame, fullBounds.size.width/2, .minXEdge)
        let (leftFrame, rightFrame) = fullBounds.divided(atDistance: fullBounds.size.width/2, from: .minXEdge)
  ...
  fileprivate static func offset(_ point: CGPoint, by offset: CGFloat) -> CGPoint {
  ...
  fileprivate func startAdd(_ viewController: UIViewController) {
  ...
  fileprivate func endAdd(_ viewController: UIViewController) {
    viewController.didMove(toParentViewController: self)
  }

  /// Start removing a child view controller.
  fileprivate func startRemove(_ viewController: UIViewController) {
  ...
  fileprivate func endRemove(_ viewController: UIViewController) {
